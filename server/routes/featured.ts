...
/**
 * POST /api/featured-listings/admin/feature-any
 * TEMPORARY: Manually feature any listing for testing
 * TODO: Remove this endpoint before production
 */
router.post("/admin/feature-any", async (req, res) => {
  try {
    const { listingId, hours } = req.body as { listingId?: string; hours?: number };
    
    // If no listing ID provided, get the first active listing
    ...
    if (!targetListingId) {
      ...
      targetListingId = firstListing.id;
      console.log(`Using first active listing: ${firstListing.title} (${targetListingId})`);
    }

    const durationHours = hours || 168; // Default 7 days
    const now = new Date();
    ...
    await db
      ...
      .set({
        featuredUntil,
        featuredPaymentId: `admin_test_${Date.now()}`,
        featuredCreatedAt: now,
        featuredDuration: `${durationHours}h`,
      })
      .where(eq(listings.id, targetListingId));

    console.log(`✅ Admin: Featured listing ${targetListingId} until ${featuredUntil}`);
