  ...
  try {
    ...
    if (apiKey) {
      try {
        ...
        // Use gemini-2.0-flash for text-only test (gemini-2.0-flash-vision requires images)
        ...
        if (geminiError.message?.includes('quota')) {
          ...
          diagnostic.diagnosis = 'MODEL_NOT_FOUND - gemini-2.0-flash-vision may not be available';
        } else {
